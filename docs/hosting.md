...
| `INFLIGHT_TTL_SECONDS`                | derived, `270`                              | How long a slot survives unreleased, for a worker killed mid-request. Derived from the timeouts rather than chosen: `SMTP_TIMEOUT_SECONDS` bounds each socket operation and a session is a dozen of them, so **raising a timeout must raise this**. A test enforces it against the configured values. |
