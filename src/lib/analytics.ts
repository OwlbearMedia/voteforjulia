...
interface NewRelicBrowserApi {
  ...
  noticeError?: (error: Error | string, customAttributes?: NewRelicAttributes) => void;
}
...
export function trackFooterIconClick(href: string, ariaLabel: string): void {
  ...
  if (normalizedHref.startsWith('mailto:') || normalizedAriaLabel.includes('email')) {
    ...
    iconLabel = 'instagram';
  } else if (normalizedAriaLabel.includes('facebook') || normalizedHref.includes('facebook.com')) {
    iconLabel = 'facebook';
  }
...
export function trackVolunteerRequestBody(requestBody: Record<string, string>): void {
  const windowObject = getWindowObject();
...
export function trackVolunteerSubmissionError(
  ...
  let normalizedError: Error | string = 'Volunteer form submission failed';
