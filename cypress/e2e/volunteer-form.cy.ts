  ...
  it('submits successfully and records the entry in Google Sheets', () => {
    ...
    cy.task<SheetRow | null>('findSheetRow', { email: TEST_EMAIL }, { timeout: 45000 }).then(
      (result) => {
        ...
        // Placement, for the same reason as the yard-sign spec — see the longer
        // note there. This worksheet was the healthy one during that outage,
        // which is exactly why it is worth pinning: it has no tracking columns
        // *today*, and the day someone adds one is the day this matters.
        expect(
          rowIndex,
